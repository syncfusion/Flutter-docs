...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/Flutter-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/Flutter-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore') {
                ...
                if (fs.existsSync('../Flutter-docs/' + changedFileName)) {
                    ...
                    if (fs.existsSync('../../../gitlabRepo/Flutter-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../Flutter-docs/` + changedFileName, `../../../gitlabRepo/Flutter-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/Flutter-docs/')) {
                            shelljs.cp('-rf', `../Flutter-docs/` + changedFileName, `../../../gitlabRepo/Flutter-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/Flutter-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/Flutter-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/Flutter-docs`);
